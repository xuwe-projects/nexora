...
/// 返回虚拟滚动股票表默认生成的行数。
///
/// 该值用于保持示例和测试中的规模一致，足够触发 `DataTable` 的垂直虚拟滚动行为。
pub fn virtual_scroll_default_row_count() -> usize {
    DEFAULT_ROW_COUNT
}

/// 返回虚拟滚动股票表的列标题。
///
/// 列标题参考 `gpui-component` 的 `data_table_story`，覆盖身份、价格变化、排名和市场数据等字段。
pub fn virtual_scroll_table_columns() -> &'static [&'static str] {
    &TABLE_COLUMNS
}
