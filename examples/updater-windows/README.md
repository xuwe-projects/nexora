...
这个独立 workspace 用来验证 Nexora 的 Windows x86_64/ARM64 首次安装和自动更新链路。首次安装生成 branded Inno Setup `.exe`，并显示简体中文现代安装向导。应用内自动更新仍只下载并应用 `windows.zip`，不会重新运行首次安装程序。
...
交互式 `nexora build` / `nexora doctor --fix` 会自动安装 Rust target、固定版本的 Inno Setup
6.7.3 和 Windows SDK。Inno 通过 winget 的官方 `JRSoftware.InnoSetup` 包安装；如果系统安装器
...
面向公众发布时建议切换到 `signing = "authenticode"`。构建会签署主程序、updater 和
...
`build` 只在本地生成 `<display_name>-<arch>.exe`、`<display_name>-<arch>.windows.zip`、
release notes、SHA-256 旁车文件和 `artifact.json`，不访问
S3。Windows 更新 ZIP 由 Rust 直接写入，归档条目统一使用 `/`，不会把本机的 `\\` 路径分隔符
带入更新协议。`publish` 只校验并发布已有产物，不隐式构建。EXE 用于首次安装，更新清单
只引用 versioned `windows.zip`。

## 真实验收

- 启动 branded `.exe`，确认显示简体中文现代安装步骤。
...
- `artifact.json` 只包含 `windows_setup_exe` 与 `windows_update_zip`；更新清单只包含 ZIP。
- 启用签名时，`signtool verify /pa` 通过主 EXE、updater EXE 和 Setup EXE。
- 不启用签名时，完整更新流程仍能通过，且不会显示必须配置 Authenticode 身份的错误。
- 从 Apps & Features 卸载时删除安装文件、安装器创建的快捷方式和卸载项，但不删除用户业务数据、配置和日志。
