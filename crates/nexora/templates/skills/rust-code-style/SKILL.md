...
- 新增或修改自定义过程宏时，宏展开后的运行时代码必须是零成本抽象：禁止不必要的堆分配、动态分发、字符串拼接和重复计算；能在编译期确定的内容不要推迟到运行时。
- 宏实现内部禁止复制粘贴重复逻辑；属性解析、诊断构造、代码生成片段和公共校验必须抽成具名 helper 函数，保持生成规则可审查。
- 宏生成的 API 优先使用具体类型；只有调用方确实需要多种类型族或 trait 边界时才引入泛型，避免把下游错误信息和编译时间复杂化。
- 宏展开代码必须通过 `cargo clippy --workspace --all-targets -- -W clippy::all`；不得依赖下游用 `allow` 消除宏展开产生的警告。
- 每个新增或修改的自定义宏都必须提供 `cargo expand` 的关键展开结果供审查，并对比同等手写实现，说明生成代码多了什么、少了什么以及为什么可接受。
- 过程宏必须包含成功用例和 `trybuild` 编译失败用例，覆盖缺失属性、类型不匹配、重复 key、非法组合等错误路径；错误消息应指向用户写错的字段或属性。
- 新增或修改自定义过程宏时，默认必须用手写等价实现作为基线运行 `cargo bench`，并用 `cargo bloat` 检查二进制体积变化；宏确实不生成运行时代码时，必须明确标注不适用并给出 `cargo expand`、Clippy 和 `trybuild` 证据。
- benchmark 未达标时，使用 `cargo asm` 检查生成汇编，区分瓶颈来自宏展开结构、下游调用方式还是无关依赖；修正后重新运行 expand、bench、bloat 与 clippy。
- `cargo expand`、`cargo bloat` 或 `cargo asm` 未安装时，优先安装后验证；确实无法安装或运行时，必须在交付说明和 Release Notes 中明确未执行项与原因。
- 宏必须保持 hygiene：生成代码使用绝对 crate 路径或稳定 re-export，不依赖调用方局部 `use`；不要生成隐藏 I/O、全局状态、后台任务或用户无法覆盖的副作用。
- 对框架增强宏，必须保留手写逃生口：调用方应能绕过宏直接实现原生 trait 或使用底层组件 API。
