CREATE TABLE account.roles (
    id BIGSERIAL CONSTRAINT roles_id_not_null NOT NULL,
    key TEXT CONSTRAINT roles_key_not_null NOT NULL,
    owner TEXT CONSTRAINT roles_owner_not_null NOT NULL DEFAULT 'IMES',
    name TEXT CONSTRAINT roles_name_not_null NOT NULL,
    description TEXT,
    is_system BOOLEAN CONSTRAINT roles_is_system_not_null NOT NULL DEFAULT FALSE,
    created_at TIMESTAMPTZ CONSTRAINT roles_created_at_not_null NOT NULL DEFAULT NOW(),
    updated_at TIMESTAMPTZ CONSTRAINT roles_updated_at_not_null NOT NULL DEFAULT NOW(),
...
COMMENT ON CONSTRAINT roles_id_not_null ON account.roles IS '要求每个角色都具有数据库主键';
COMMENT ON CONSTRAINT roles_key_not_null ON account.roles IS '要求每个角色都具有稳定授权键';
COMMENT ON CONSTRAINT roles_owner_not_null ON account.roles IS '要求每个角色都具有所属范围';
COMMENT ON CONSTRAINT roles_name_not_null ON account.roles IS '要求每个角色都具有展示名称';
COMMENT ON CONSTRAINT roles_is_system_not_null ON account.roles IS '要求每个角色都明确是否为系统角色';
COMMENT ON CONSTRAINT roles_created_at_not_null ON account.roles IS '要求每个角色都记录创建时间';
COMMENT ON CONSTRAINT roles_updated_at_not_null ON account.roles IS '要求每个角色都记录最后更新时间';
...
CREATE TABLE account.permissions (
    id BIGSERIAL CONSTRAINT permissions_id_not_null NOT NULL,
    key TEXT CONSTRAINT permissions_key_not_null NOT NULL,
    name TEXT CONSTRAINT permissions_name_not_null NOT NULL,
    description TEXT,
    created_at TIMESTAMPTZ CONSTRAINT permissions_created_at_not_null NOT NULL DEFAULT NOW(),
...
COMMENT ON CONSTRAINT permissions_id_not_null ON account.permissions IS '要求每个权限都具有数据库主键';
COMMENT ON CONSTRAINT permissions_key_not_null ON account.permissions IS '要求每个权限都具有稳定授权键';
COMMENT ON CONSTRAINT permissions_name_not_null ON account.permissions IS '要求每个权限都具有展示名称';
COMMENT ON CONSTRAINT permissions_created_at_not_null ON account.permissions IS '要求每个权限都记录创建时间';
...
CREATE TABLE account.user_roles (
    user_id VARCHAR(8) CONSTRAINT user_roles_user_id_not_null NOT NULL,
    role_id BIGINT CONSTRAINT user_roles_role_id_not_null NOT NULL,
    granted_by VARCHAR(8),
    created_at TIMESTAMPTZ CONSTRAINT user_roles_created_at_not_null NOT NULL DEFAULT NOW(),
...
COMMENT ON CONSTRAINT user_roles_user_id_not_null ON account.user_roles IS '要求每条用户角色关系都引用用户';
COMMENT ON CONSTRAINT user_roles_role_id_not_null ON account.user_roles IS '要求每条用户角色关系都引用角色';
COMMENT ON CONSTRAINT user_roles_created_at_not_null ON account.user_roles IS '要求每条用户角色关系都记录创建时间';
...
CREATE TABLE account.role_permissions (
    role_id BIGINT CONSTRAINT role_permissions_role_id_not_null NOT NULL,
    permission_id BIGINT CONSTRAINT role_permissions_permission_id_not_null NOT NULL,
    created_at TIMESTAMPTZ CONSTRAINT role_permissions_created_at_not_null NOT NULL DEFAULT NOW(),
...
COMMENT ON CONSTRAINT role_permissions_role_id_not_null ON account.role_permissions IS '要求每条角色权限关系都引用角色';
COMMENT ON CONSTRAINT role_permissions_permission_id_not_null ON account.role_permissions IS '要求每条角色权限关系都引用权限';
COMMENT ON CONSTRAINT role_permissions_created_at_not_null ON account.role_permissions IS '要求每条角色权限关系都记录创建时间';
...
CREATE TABLE account.permission_implications (
    permission_id BIGINT CONSTRAINT permission_implications_permission_id_not_null NOT NULL,
    implied_permission_id BIGINT CONSTRAINT permission_implications_implied_permission_id_not_null NOT NULL,
    created_at TIMESTAMPTZ CONSTRAINT permission_implications_created_at_not_null NOT NULL DEFAULT NOW(),
...
COMMENT ON CONSTRAINT permission_implications_permission_id_not_null ON account.permission_implications IS '要求每条蕴含关系都引用上游权限';
COMMENT ON CONSTRAINT permission_implications_implied_permission_id_not_null ON account.permission_implications IS '要求每条蕴含关系都引用被蕴含权限';
COMMENT ON CONSTRAINT permission_implications_created_at_not_null ON account.permission_implications IS '要求每条蕴含关系都记录创建时间';
