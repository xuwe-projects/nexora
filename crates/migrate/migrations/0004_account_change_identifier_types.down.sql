...
ALTER TABLE account.system_initialization
    ADD COLUMN super_admin_user_id_uuid UUID;

UPDATE account.system_initialization AS initialization
SET super_admin_user_id_uuid = users.id_uuid
FROM account.users AS users
...
ALTER TABLE account.users
    ...
    DROP CONSTRAINT users_id_format;
ALTER TABLE account.roles DROP CONSTRAINT roles_pkey;
ALTER TABLE account.permissions DROP CONSTRAINT permissions_pkey;

ALTER SEQUENCE account.roles_id_seq OWNED BY NONE;
ALTER SEQUENCE account.permissions_id_seq OWNED BY NONE;

ALTER TABLE account.system_initialization DROP COLUMN super_admin_user_id;
ALTER TABLE account.system_initialization
    RENAME COLUMN super_admin_user_id_uuid TO super_admin_user_id;

ALTER TABLE account.user_roles
    DROP COLUMN user_id,
    DROP COLUMN role_id,
    DROP COLUMN granted_by;
ALTER TABLE account.user_roles RENAME COLUMN user_id_uuid TO user_id;
ALTER TABLE account.user_roles RENAME COLUMN role_id_uuid TO role_id;
ALTER TABLE account.user_roles RENAME COLUMN granted_by_uuid TO granted_by;

ALTER TABLE account.role_permissions
    DROP COLUMN role_id,
    DROP COLUMN permission_id;
ALTER TABLE account.role_permissions RENAME COLUMN role_id_uuid TO role_id;
ALTER TABLE account.role_permissions RENAME COLUMN permission_id_uuid TO permission_id;

ALTER TABLE account.users DROP COLUMN id;
ALTER TABLE account.users RENAME COLUMN id_uuid TO id;

ALTER TABLE account.roles DROP COLUMN id;
ALTER TABLE account.roles RENAME COLUMN id_uuid TO id;

ALTER TABLE account.permissions DROP COLUMN id;
ALTER TABLE account.permissions RENAME COLUMN id_uuid TO id;

DROP SEQUENCE account.roles_id_seq;
DROP SEQUENCE account.permissions_id_seq;
...
COMMENT ON COLUMN account.users.id IS '回滚时重新生成的稳定用户 UUID 主键；原 8 位用户 ID 无法恢复';
COMMENT ON CONSTRAINT users_pkey ON account.users IS '保证每个本地用户具有唯一稳定主键';

COMMENT ON COLUMN account.roles.id IS '回滚时重新生成的稳定角色 UUID 主键；原 BIGSERIAL ID 无法恢复';
COMMENT ON CONSTRAINT roles_pkey ON account.roles IS '保证每个角色具有唯一稳定主键';

COMMENT ON COLUMN account.permissions.id IS '回滚时重新生成的稳定权限 UUID 主键；原 BIGSERIAL ID 无法恢复';
COMMENT ON CONSTRAINT permissions_pkey ON account.permissions IS '保证每个权限具有唯一稳定主键';

COMMENT ON COLUMN account.user_roles.user_id IS '获得角色的用户 UUID';
COMMENT ON COLUMN account.user_roles.role_id IS '直接授予用户的角色 UUID';
COMMENT ON COLUMN account.user_roles.granted_by IS '执行角色授予的本地用户 UUID，授权人删除后保留空值';
...
COMMENT ON COLUMN account.role_permissions.role_id IS '获得权限的角色 UUID';
COMMENT ON COLUMN account.role_permissions.permission_id IS '授予角色的权限 UUID';
...
COMMENT ON COLUMN account.system_initialization.super_admin_user_id IS '初始化时选定的超级管理员本地用户 UUID';
