...
fn write_publish_cargo_package(directory: &TestDirectory) {
    fs::create_dir_all(directory.path().join("src")).unwrap();
    fs::create_dir_all(directory.path().join("docs/releases/current")).unwrap();
    fs::write(
    ...
    fs::write(
        directory.path().join("docs/releases/current/zh-CN.md"),
        "# 1.0.1\n\n- 发布测试更新日志。\n",
    )
    .unwrap();
    fs::write(
...
fn write_publish_receipt(directory: &TestDirectory, base: &str) {
    ...
    fs::write(
        ...
        serde_json::to_vec_pretty(&serde_json::json!({
            ...
            "notes_source": "docs/releases/current/zh-CN.md"
        }))
        .unwrap(),
    )
    .unwrap();
    fs::create_dir_all(directory.path().join("dist/desktop/stable/1.0.1/12")).unwrap();
    fs::write(
        directory
            .path()
            .join("dist/desktop/stable/1.0.1/12/notes.md"),
        "# 1.0.1\n\n- 发布测试更新日志。\n",
    )
    .unwrap();
}
...
fn signed_remote_manifest(seed: [u8; 32], sequence: u64) -> Vec<u8> {
    signed_remote_manifest_identity(seed, sequence, "1.0.0", 1)
}

fn signed_remote_manifest_identity(
    seed: [u8; 32],
    sequence: u64,
    version: &str,
    build_number: u64,
...
"#
    ...
    assert_eq!(
        latest["payload"]["notes_sha256"],
        format!("{:x}", Sha256::digest(&notes))
    );
    assert!(
...
#[test]
fn publish_reads_dynamic_identity_from_receipt_without_recomputing_time() {
    let directory = TestDirectory::new("publish-frozen-dynamic-identity");
    let store = MockObjectStore::new();
    let base = store.base_url();
    ...
    let config = fs::read_to_string(&config_path)
        .unwrap()
        .replace("version = \"1.0.1\"", "version = \"${CARGO_PKG_VERSION}\"")
        .replace("build_number = 12", "build_number = \"${BUILD_DATETIME}\"");
    fs::write(config_path, config).unwrap();
    let receipt_path = directory.path().join("dist/desktop/stable/release.json");
    let mut receipt: serde_json::Value =
        serde_json::from_slice(&fs::read(&receipt_path).unwrap()).unwrap();
    receipt["version_source"] = "cargo_pkg_version".into();
    receipt["build_number_source"] = "build_datetime".into();
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("版本：1.0.1（cargo_pkg_version）"));
    assert!(stdout.contains("Build：12（build_datetime）"));
    assert!(store.puts().is_empty());
}

#[test]
fn publish_rejects_corrupt_receipt_before_upload() {
    let directory = TestDirectory::new("publish-corrupt-receipt");
    let store = MockObjectStore::new();
    let base = store.base_url();
    prepare_publish_app(&directory, &base);
    fs::write(
    ...
    assert!(String::from_utf8_lossy(&output.stderr).contains("release receipt"));
    assert!(store.puts().is_empty());
}

#[test]
fn publish_rejects_artifact_mismatch_before_upload() {
    let directory = TestDirectory::new("publish-artifact-mismatch");
    let store = MockObjectStore::new();
    let base = store.base_url();
    prepare_publish_app(&directory, &base);
    fs::write(
    ...
    assert!(!output.status.success());
    assert!(String::from_utf8_lossy(&output.stderr).contains("artifact.json 不一致"));
    assert!(store.puts().is_empty());
}

#[test]
fn publish_rejects_remote_identity_that_is_not_lower() {
    let directory = TestDirectory::new("publish-remote-not-lower");
    let store = MockObjectStore::new();
    let base = store.base_url();
    let seed = prepare_publish_app(&directory, &base);
    let latest_path = "/desktop-releases/e2e-test/desktop/stable/latest.json";
    store.insert(
        latest_path,
        signed_remote_manifest_identity(seed, 4, "1.0.1", 12),
    );

    let output = directory.run(&["publish", "--app", "desktop", "--dry-run"]);

    assert!(!output.status.success());
    assert!(String::from_utf8_lossy(&output.stderr).contains("必须严格高于"));
    assert!(store.puts().is_empty());
}
