...
- Before compiling or writing a release receipt, build performs the same read-only host dependency checks in interactive and non-interactive terminals. It never downloads tools, runs package-manager installation commands, opens download pages, or changes PATH. Missing Rust targets, Cargo packaging tools, Xcode/CLT, Windows SDK, or compatible Inno Setup (`>= 6.7.3, < 8.0.0`) fail with official links and copyable manual commands; after installing them, the user reruns the original build command.
...
- Windows x86_64/ARM64: a Simplified Chinese Inno Setup EXE for first installation and an update ZIP for the sidecar. Both artifacts are built from the same staging directory. Generate separate UTF-8 RC files for main/updater with `#pragma code_page(65001)`, string table `080404B0` and Translation `0x0804,1200`. Main `FileDescription` is `display_name`; updater appends “更新程序”; `InternalName`/`OriginalFilename` retain package identities. Link each resource before Authenticode signing. Installation is fixed to the current user under `%LOCALAPPDATA%\Programs\<publisher>\<display_name>` without UAC and preserves the install-directory chooser. Both `publisher` and `display_name` must pass Windows directory-component validation. `app_id` remains the stable Inno/updater/publish identity and must not replace either user-visible default directory component. The UI exposes desktop shortcut, Start menu shortcut, and launch-after-finish checkboxes. Main and sidecar executables use the Windows GUI subsystem. `signing = "none"` still enforces Ed25519, artifact size/SHA-256, ZIP safety and PE architecture checks but skips Authenticode; Authenticode-only TOML fields conflict with that mode. `signing = "authenticode"` signs the main executable, sidecar, and final Setup EXE, and requires the updater to verify both staged EXEs with Windows trust, certificate thumbprint and publisher. The bundled thumbprint and publisher must be both absent or both present. The Rust ZIP writer must encode archive entry names with `/`; do not use PowerShell `Compress-Archive` or serialize native Windows `Path` separators into the update protocol. Inno is never run during sidecar updates. The current pinned GPUI floor is Windows 10 1703/build 15063.
...
On Windows, publishing `pending.json` must use a synced temporary file plus an atomic replace that can overwrite an existing destination. Directory durability is best-effort after that commit and must never turn a committed record into an error that moves its payload back to staging. The verified main EXE filename must be passed explicitly through the helper command and used for preflight, launch, and failure relaunch; never infer it by scanning other EXEs in the install directory. After switching directories, copy the existing top-level Inno Setup `unins<digits>.exe`, `.dat`, and `.msg` files from the backup into the new installation before launch, reject staged collisions, and keep the originals in the backup until health confirmation so rollback and Apps & Features uninstall both remain valid. A failed transaction records a bounded, user-safe result before restarting the restored version; the next launch consumes that result through the shared notification UI.
