...
- Nexora 历史固定在 `nexora._sqlx_migrations`，应用历史由应用 Migrator 独立维护；不要增加
  需要人工切换的首次初始化布尔配置。数据库测试必须保留 `#[sqlx::test]`，并从项目标准
  `config/server.toml` 为当前测试进程派生 `DATABASE_URL`。
