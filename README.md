...
> 变化，暂不建议直接用于生产环境。`0.1.0` 是首个 GitHub 源码版本，当前尚未发布到
> crates.io。
...
不带参数运行 `nexora create` 时，CLI 会交互询问项目名称、布局和是否启用 Account。
`single` 会在项目根目录生成一个最小桌面 package；`workspace` 会把桌面端生成到
`apps/<项目名>` 并将其设为默认 workspace member。需要完整认证授权、用户、角色与权限能力时：

```bash
nexora create my-fullstack-app --layout workspace --features account
```

Account 项目还会生成 `apps/server` 和桌面端、服务端配置；启动前按照生成项目的
`README.md` 配置 PostgreSQL 与 OIDC。CLI 会让生成项目固定依赖同版本 Git tag，避免跟随
`main`；首次构建后应提交生成的 `Cargo.lock`，CI 和部署使用 `--locked` 保持完整依赖图稳定。

手动创建应用时，框架库与 CLI 使用同一个 Cargo package 名：

```toml
[dependencies]
nexora = { git = "https://github.com/xuwe-projects/nexora", tag = "v0.1.0", default-features = false, features = ["desktop", "derive"] }
```
...
- 同一个 CLI 提供 `create`、`init`、workspace `lint`，以及 macOS 的 `build`、`doctor`，
  不再维护第二个工具二进制；
...
因此应用不需要维护两份渲染代码。返回值会成为 Panel 内容；通用 Shell 默认提供 `24px`
内容内边距和纵向滚动，不会擅自添加业务卡片。页面内部已经管理滚动时，可设置
`content_scrollable = false` 关闭外层滚动。动态路径必须声明
...
在仓库中调试 CLI 时，可以安装当前工作树：

```bash
cargo install --path crates/nexora --locked \
  --no-default-features --features cli --bin nexora
```

精确的桌面 dev profile 在这个 CLI-only 隔离依赖图中会提示部分 UI package 未匹配；它不影响
CLI 安装结果，后续会在不扩大第三方优化范围的前提下单独处理。
...
.
├── examples/
│   ├── basic-workspace/        # 无 Account 的最小 workspace 示例
│   ├── console/                # GPUI 桌面端完整集成示例
│   └── server/                 # Axum、PgPool 与 Account 组合示例
├── config/
│   └── example.server.toml      # 可提交的服务端配置示例
├── crates/
│   ├── nexora/                 # 框架库、CLI、项目模板与内置 Skills
│   ├── macros/                 # Feature、Window、Settings 等派生宏
