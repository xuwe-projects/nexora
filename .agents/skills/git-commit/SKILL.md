---
name: git-commit
description: 在 Wisdom workspace 中准备、审查或创建 Git 提交时使用。强制使用 Conventional Commits v1.0.0 格式，提交内容使用中文，并要求正文详细说明改动内容、目的、影响范围和验证情况。
---
...
使用这个 skill 为 Wisdom workspace 创建清晰、可审阅的 Git 提交。提交信息必须遵循 Conventional Commits v1.0.0，同时人类可读的摘要和正文使用中文。
...
当 scope 有助于定位影响范围时使用 scope，例如 `console`、`assets`、`migrate`、`miniprogram`、`skills`，或 `enterprises` 这类业务模块。
...
```sh
git commit \
  -m "feat(enterprises): 增加企业详情页" \
  -m "改动内容：
- 企业列表增加详情入口和分页信息。
- 新增企业详情页，用于集中展示基础信息、联系人和关联日程占位。

目的：
- 让联系人回到企业上下文中维护，避免联系人作为独立菜单造成信息架构割裂。

影响范围：
- 控制台企业列表、企业详情路由和企业联系人表单。
