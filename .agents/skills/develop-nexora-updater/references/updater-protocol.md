...
- In an interactive terminal, build repairs missing host dependencies before compiling, including Rust targets, Cargo packaging tools, macOS Homebrew tools, pinned Inno Setup 6.7.3, and the Windows SDK. System installers such as Xcode Command Line Tools and Windows SDK may pause the build and require rerunning the same command. Non-interactive builds fail with exact installation commands instead of starting installers.
