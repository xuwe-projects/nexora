...
`FormDialog` 是创建/编辑资源的默认表单容器，由三个固定区域组成：标题与可选描述、表单内容、
取消与提交操作。它组合 `PanelDialog`，遮罩只覆盖当前 Feature 内容区，用户仍可操作 Sidebar
与其他菜单；表单不会通过点击遮罩关闭，取消、右上角关闭和提交才是明确意图。
...
渲染时使用 builder API 组合标准字段和自定义区域。`FormItem` 负责字段标签、说明、必填标记
和控件，内置 `input`、`password_input`、`number_input`、`checkbox`，复杂内容用
`element` 或 `FormDialog::section`：
...
`on_submit` 没有默认业务行为，调用方必须实现；可用 `description`、`cancel_label`、
`submit_label`、`submit_disabled` 和 `on_cancel` 定制。`submit_disabled(true)` 只禁用提交按钮，
取消仍可用于退出或处理草稿；只有 `set_submitting(true)` 会同时禁用取消、关闭和重复提交。
默认取消会检查全部草稿：无修改时关闭，有修改时列出未保存字段与当前草稿并要求确认。自定义
取消可以从同一个状态读取：
...
数据；查询、创建、导入、导出和批量操作放在 `CrudPanelToolbar` 的 action 区。页面应把
`theme::component_size(cx)` 传给 `CrudPanel`、工具栏按钮、输入和下拉，让设置中的组件尺寸
即时生效。
