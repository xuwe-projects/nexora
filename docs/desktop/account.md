...
- 人员与 Provider 服务账号的统一列表，以及独立的服务账号创建/同步入口；
...
默认用户列表只显示绑定 Provider `identity_id` 的人员与服务账号。`identity_id = null` 的内部
服务主体用于审计等本地系统行为，不登录也不会出现在默认列表；无需新增第三种用户类型。

服务账号使用独立 `FormDialog`，不会把人员字段混进同一个切换表单。表单提交 username、
展示名称、说明和初始角色；同 username machine user 已存在时，使用官方确认对话框提示
“是否直接同步并使用”。确认后客户端以 `use_existing = true` 重试，Provider-only 账号会
同步到本地 Account，并按当前选择完整替换本地与 ZITADEL Project 角色。若同 username 属于
人员账号，则展示稳定类型冲突错误。

默认用户页面不再提供“账号与凭据”入口，也不创建、列出、轮换或撤销 Secret/PAT。开发或
运维人员直接在 ZITADEL 后台管理服务账号凭据；这不影响服务端继续验证 JWT 或在配置
introspection Client ID/Secret 后验证 opaque PAT。

默认角色页面对 `IMES` owner 的角色定义执行 ZITADEL 双写；用户管理中的角色添加、移除和
完整替换也同步同一 Project authorization。服务账号仍使用统一角色与状态操作，不自动获得
`member`，也不提供删除操作。
