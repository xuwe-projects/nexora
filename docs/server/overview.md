...
`users:roles.write`。请求提交 username、人类用户姓名和邮箱，服务端通过 ZITADEL UserService
v2 gRPC 创建身份，再使用返回的 identity ID 原子绑定本地用户与初始角色；不接受客户端提供
裸 identity ID。`GET /me` 会按 token subject 从 ZITADEL 同步最新资料，但仍只允许已经开通且
启用的本地用户。可信宿主直接调用 pool-first API 时必须自行执行等价授权。
