...
| --- | --- | --- | --- |
| `id` | int64 | 否 | PostgreSQL BIGSERIAL ID |
| `owner` | string | 否 | 角色所属范围；`IMES` 为默认后台系统范围，客户门户可使用客户 ID |
| `key` | string | 否 | 稳定角色键 |
...
| `initial_password` | string | 是 | 1 至 200 个字符；只写入 ZITADEL |
| `require_password_change` | boolean | 否 | 默认 `false`；是否要求用户首次登录后修改密码 |
...
权限：`roles:read`。可选 query `owner`，缺省为 `IMES`。成功返回：

```json
{ "items": [{ "id": 2, "owner": "IMES", "key": "member", "name": "成员", "description": null, "is_system": true, "permissions": [], "created_at": 1784304000, "updated_at": 1784304000 }] }
```
...
| body 字段 | 类型 | 必填 | 约束 |
| --- | --- | --- | --- |
| `owner` | string | 否 | 缺省 `IMES`；最多 200 个字符 |
...
权限：`roles:read`。`role_id` 是大于零的 int64；可选 query `owner` 缺省为 `IMES`。
成功返回 `200 Role`；该角色不属于指定 owner 时按不存在返回 `404`；
无法解析的 path 返回 `400 invalid_path_parameter`。

### `PATCH /roles/{role_id}`

权限：`roles:write`。可选 query `owner` 缺省为 `IMES`。至少提供一个字段：
...
权限：`roles:write`。`owner` 缺省为 `IMES`；`permission_ids` 是替换后的完整直接权限集合：
