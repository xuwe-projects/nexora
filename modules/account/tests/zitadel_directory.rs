...
fn directory_requires_https_except_for_loopback_development() {
    assert!(
        ZitadelUserDirectory::new("http://id.example.com", TEST_TOKEN, TEST_PROJECT_ID).is_err()
    );
    assert!(
        ZitadelUserDirectory::new("https://id.example.com", TEST_TOKEN, TEST_PROJECT_ID).is_ok()
    );
    assert!(
        ZitadelUserDirectory::new("http://localhost:8080", TEST_TOKEN, TEST_PROJECT_ID).is_ok()
    );
    assert!(
        ZitadelUserDirectory::new("http://127.0.0.1:8080", TEST_TOKEN, TEST_PROJECT_ID).is_ok()
    );
}

#[test]
fn directory_rejects_invalid_issuer_and_pat() {
    assert!(ZitadelUserDirectory::new("not-an-url", TEST_TOKEN, TEST_PROJECT_ID).is_err());
    assert!(
    ...
    assert!(ZitadelUserDirectory::new("https://id.example.com", "  ", TEST_PROJECT_ID).is_err());
    assert!(
        ZitadelUserDirectory::new("https://id.example.com", "invalid\npat", TEST_PROJECT_ID,)
            .is_err()
    );
    assert!(ZitadelUserDirectory::new("https://id.example.com", TEST_TOKEN, "  ").is_err());
}

#[tokio::test]
async fn explicit_identity_id_must_not_be_empty() {
    let directory = ZitadelUserDirectory::new("http://localhost:8080", TEST_TOKEN, TEST_PROJECT_ID)
