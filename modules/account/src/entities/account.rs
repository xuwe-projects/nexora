...
impl PermissionKey {
    ...
    /// 在身份 Provider 与本地账号目录创建服务账号。
    pub const ServiceAccountsProvision: Self = Self(Cow::Borrowed("service_accounts:provision"));
    /// 修改服务账号展示名称和说明。
...
/// PostgreSQL `account.user_type` 对应的账号主体类型。
#[derive(Debug, Clone, Copy, PartialEq, Eq, Type)]
#[sqlx(type_name = "account.user_type", rename_all = "snake_case")]
pub enum UserType {
    /// 通过交互式登录代表自然人的账号。
    Human,
